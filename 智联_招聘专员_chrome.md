...
Do While sRet<>0 
	...
	For y = 1 To iRetFather Step 1
		...
		// 2. 学历（大专|本科）
		cetification =  Regex.FindStr(waittingData[y-1][2],'大专|本科',0)
		...
		// 4. 求职期望(含专员/助理)
		expectJob =  Regex.FindStr(waittingData[y-1][4],'专员|助理|招聘',0)
		TracePrint("源文本："& waittingData[y-1][4] &"匹配后显示："& expectJob & "--求职期望")
		
		// 5. 工资最小值(10,000)
		arrRet =  Split(waittingData[y-1][5],"-")
		num =  Regex.FindStr(arrRet[0],'[1-9]\\d*\\.?\\d*',0)
		thousand =  Regex.FindStr(arrRet[0],'千',0)
		...
		Else
			minSalary = (num)*10000
		End If
		TracePrint("源文本："& waittingData[y-1][5] &"匹配后显示："& minSalary & "--工资最小值")
		#icon("@res:vo60278g-n0nt-jhsh-vspf-ak92gpvq7n7e.png")
	
		// // 6.求职简介
		// summary = Regex.FindStr(waittingData[y-1][5],"外包|外派",0)
		...
		If age <=32 And cetification <>"" And work_status <>"" And expectJob <> "" And minSalary <=10000
			// 滚动以防止鼠标悬浮在电脑底部菜单栏
            Mouse.Wheel(3,"down", [],{"iDelayAfter":300,"iDelayBefore":200})
			#icon("@res:e0sbkpmi-c69o-0ees-efkc-vk6fguqovajp.png")
			Text.Click({"wnd":[{"cls":"Chrome_WidgetWin_1","title":"*","app":"chrome"},{"cls":"Chrome_RenderWidgetHostHWND","title":"Chrome Legacy Window"}],"html":[{"tag":"SPAN","aaname":"立即聊","idx":7}]},"立即聊","instr",1,"left","click",10000,{"bContinueOnError":False,"iDelayAfter":300,"iDelayBefore":200,"bSetForeground":True,"sCursorPosition":"Center","iCursorOffsetX":0,"iCursorOffsetY":0,"sKeyModifiers":[],"sSimulate":"simulate"})
